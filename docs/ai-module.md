...
CollabAI integrates an external **LLM API (Groq)** for chatbot-style Q&A, text analysis, project plan generation, and daily standup narratives. All features are exposed under **`/api/v1/ai/`** and documented in Swagger (`/api/docs/`).
...
| `GET` | `/ai/config/` | LLM configuration status |
| `POST` | `/ai/task-generator/plans/` | AI project plan generation |
