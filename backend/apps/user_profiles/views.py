...
class UserViewSet(viewsets.ReadOnlyModelViewSet):
	...
	def get_queryset(self) -> QuerySet[User]:
		...
		if getattr(user, 'is_superuser', False):
			return User.objects.all().select_related('profile', 'profile__workspace', 'profile__role').order_by('email')

		workspace_ids = workspaces_queryset_for_user(user).values_list('pk', flat=True)
		return (
			User.objects.filter(Q(workspace_memberships__workspace_id__in=workspace_ids) | Q(pk=user.pk))
			.distinct()
			.select_related('profile', 'profile__workspace', 'profile__role')
			.order_by('email')
		)
